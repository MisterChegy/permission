...
function disposeTree(data) {
	...
	$('#tt').tree({
		data : data,
		onContextdepart : function(e, node) {
			e.preventDefault();
...
/**
 * 加载表格数据 departmentid：菜单id
 */
function loadDataGrid(departmentid) {
	$('#grid').datagrid({
		url : 'depart/departfindById?departmentid=' + departmentid,
		columns : [ [ {
			field : 'departmentid',
			title : '编号',
			width : 100
...
function insertData() {
	...
	formData.pid = rowData.departmentid;
...
function deleteData(departmentid) {
	...
	$.messager.confirm('警告', '确认要删除“' + gridData.departname + '”菜单吗?',
			function(r) {
				if (r) {
					$.ajax({
						type : 'POST',
						url : 'depart/departdelete',
						data : {
...
function renamemenu() {
	...
	formData.departmentid = $('#tt').tree('getSelected').id;
	$.ajax({
		type : 'POST',
		url : 'depart/departupdateById',
		data : formData,
		dataType : 'json',
		success : function(data) {
			$.messager.alert("提示", data.msg, 'info', function() {
